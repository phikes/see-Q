...
export const DriverInput = () => {
  const { setFieldValue, values: { driver, vendor, transceiver } } = useFormikContext<TransceiverConfig>()
  const driverTypes = find(Transceivers, {deviceVendor: vendor, deviceName: transceiver})?.supportedDrivers ?? []

  const filteredDriverTypes = driverTypes.filter((driverType) => (SUPPORTED_DRIVER_TYPES as readonly DriverType[]).includes(driverType)) as Array<typeof SUPPORTED_DRIVER_TYPES[number]>
  ...
  }, [driver, setFieldValue])

  const id = useId()
  const textId = useId()

  return <Form.Group controlId={id} className="mb-3">
    <Form.Label><FormattedMessage defaultMessage="Driver" /></Form.Label>
  ...
  <Form.Text className="d-inline-block mb-3" id={textId}><FormattedMessage defaultMessage="The WebSerial driver should be supported on a wide variety of platforms. You might need to install a driver for your transceiver before it becomes available in the list of devices your browser presents to you after submitting this form. Try a WebUSB-based driver on mobile devices if the WebSerial driver is not working." /></Form.Text>
